...
class AfterTradeEntry(models.Model):
    ...
    bias = models.CharField(max_length=10, choices=BIAS_CHOICES, blank=True, null=True)
    htf_mitigation = models.BooleanField(default=False)
    liquidity_analysis = models.CharField(max_length=50, choices=LIQUIDITY_ANALYSIS_CHOICES, blank=True, null=True)
    market_condition = models.CharField(max_length=20, choices=MARKET_CONDITION_CHOICES, blank=True, null=True)
    lower_tf_confirmation = models.CharField(max_length=10, choices=LOWER_TF_CONFIRMATION_CHOICES, blank=True, null=True)
    ...
    predicted_directional_bias = models.CharField(max_length=10, choices=PREDICTED_BIAS_CHOICES, blank=True, null=True)
    poi_performance = models.CharField(max_length=20, choices=POI_PERFORMANCE_CHOICES, blank=True, null=True)
    risk_percentage = models.DecimalField(max_digits=5, decimal_places=2, blank=True, null=True, default=0)
    market_behaviour = models.CharField(max_length=30, choices=MARKET_BEHAVIOUR_CHOICES, blank=True, null=True)
    entry_quality = models.CharField(max_length=20, choices=ENTRY_QUALITY_CHOICES, blank=True, null=True)
    poi_quality_score = models.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(5)], blank=True, null=True)
    time_of_entry = models.TimeField(blank=True, null=True)
    outcome = models.CharField(max_length=10, choices=OUTCOME_CHOICES)
    is_win = models.BooleanField(default=False, editable=False)
    discipline_score = models.CharField(max_length=20, choices=DISCIPLINE_SCORE_CHOICES, blank=True, null=True)
